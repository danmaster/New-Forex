      ...
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
        {
         if(OrderMagicNumber() == MagicNumber && OrderSymbol() == Symbol())
           {
            // Evitar múltiples cierres parciales de forma robusta e independiente del broker
            ...
            for(int j = OrdersHistoryTotal() - 1; j >= 0; j--)
              {
               if(OrderSelect(j, SELECT_BY_POS, MODE_HISTORY))
                 {
                  if(OrderMagicNumber() == MagicNumber && OrderSymbol() == Symbol() && OrderOpenTime() == openT)
                    {
            ...
            // Reseleccionamos la orden original porque el bucle anterior movió el puntero
            OrderSelect(i, SELECT_BY_POS, MODE_TRADES); 
            
            if(alreadyPartiallyClosed) continue; 
...
int GetTradesTodayCount()
  {
   ...
   datetime uniqueOpenTimes[100];
   
   // Helper lambda-like para buscar si el tiempo ya está en el array
   // Como MQL4 no tiene lambdas, lo hacemos inline en los bucles
      ...
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
        {
         if(OrderMagicNumber() == MagicNumber && OrderSymbol() == Symbol())
           {
            if((OrderType() == OP_BUY || OrderType() == OP_SELL) && OrderOpenTime() >= dayStart)
              {
               datetime ot = OrderOpenTime();
               bool found = false;
               for(int k = 0; k < count; k++) { if(uniqueOpenTimes[k] == ot) { found = true; break; } }
               if(!found && count < 100)
                 {
                  uniqueOpenTimes[count] = ot;
                  count++;
                 }
              }
           }
...
bool isBearishReversal(int shift)
  {
   ...
   if(body > 0)
     ...
     {
      // Si es un Doji plano
      if(upperWick > 2.0 * Point * 10 && lowerWick < upperWick * 0.5) isPinbar = true;
     }
...
bool isBullishReversal(int shift)
  {
   ...
   if(body > 0)
     ...
     {
      // Si es un Doji plano
      if(lowerWick > 2.0 * Point * 10 && upperWick < lowerWick * 0.5) isPinbar = true;
     }
   ...
   if(currentHour >= EndHour && currentHour < 23)
     {
      ...
      // Sumamos 1 para incluir tanto shiftStart como shiftEnd en el conteo de iHighest/iLowest
            ...
            if(Close[1] > asianHigh)
              {
               if(!sweptHigh)
                 {
                  double bodyPips = MathAbs(Close[1] - Open[1]) / (10 * Point);
