...
input int      MaxCandlesOutside = 3;          // Max velas fuera de caja antes de reversión
         ...
         if(isNewBar)
           {
            ...
            // 1.5 Validar si la trampa ha caducado por tiempo o velas
            if(sweptHigh)
              {
               int barsPassed = iBarShift(Symbol(), Period(), timeSweptHigh);
               if(barsPassed > MaxCandlesOutside || (Time[1] - timeSweptHigh > MaxMinutesForReversal * 60))
                 {
                  sweptHigh = false;
                  Print("SMC: Trampa Alcista (Sweep High) CADUCADA. Velas fuera: ", barsPassed);
                 }
              }
            if(sweptLow)
              {
               int barsPassed = iBarShift(Symbol(), Period(), timeSweptLow);
               if(barsPassed > MaxCandlesOutside || (Time[1] - timeSweptLow > MaxMinutesForReversal * 60))
                 {
                  sweptLow = false;
                  Print("SMC: Trampa Bajista (Sweep Low) CADUCADA. Velas fuera: ", barsPassed);
                 }
              }
