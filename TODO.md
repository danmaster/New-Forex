...
- `PauseAfterLossStreak` agregado (default 0, OFF). No habilitado: con `MaxTradesPerDay=1` salta días vacíos y su chequeo diario de historia tiene bug de lockout (se re-pausa cada día si no hay un win para resetear el streak). **REESCRITO EN V2.71b**: contador en memoria + cooldown fijo (`StreakCooldownDays`) + ventana `StreakLookbackDays`. Verificado en `Asian_V2.70_3.htm`: con =2, neto +769 / 73.3% (proyección, A/B test).
