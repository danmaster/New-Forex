...
input bool     UseTrailingStop   = true;       // Activar Trailing Stop (False = Purista)
...
input bool     UseAutoBreakEven    = true;       // Activar Auto Break-Even (False = Purista)
...
input bool     UsePartialClose     = true;       // Activar Cierre Parcial Automático (False = Purista)
         ...
         if(ObjectFind(0, ManualBoxName) >= 0)
           {
            ...
            if(boxBottom >= asianHigh)
              {
               ...
               double slPips = (stopLoss - limitPrice) / (10 * Point);
            ...
            // Si la caja manual esta por debajo de la asiatica, queremos COMPRAR
            else if(boxTop <= asianLow)
              {
               ...
               double slPips = (limitPrice - stopLoss) / (10 * Point);
               if(UseFixedRR) takeProfit = NormalizeDouble(limitPrice + (slPips * 10 * Point * FixedRiskReward), Digits);
