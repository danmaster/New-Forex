...
input int      MinBoxPips        = 15;         // Tamaño Mínimo Caja (pips) [Alex Ruiz: <10 = mercado muerto]
...
input int      MaxTradeHour      = 13;         // Cierre Forzoso y fin de búsqueda (13:00 Skilling = 12:00 Madrid)
...
input bool     UseTrailingStop   = true;       // Activar Trailing Stop (Activado V1.015)
input int      TrailingPips      = 15;         // Distancia del Trailing (pips)

input string   StopLoss_Settings = "--- AJUSTES DE STOP LOSS ---";
input int      SLBufferPips      = 8;          // Pips extra de margen sobre el sweep
input int      MinSLPips         = 15;         // Stop Loss mínimo (pips) — nunca menos de 15 pips

input string   Management_Settings = "--- GESTION DE OPERACIONES ---";
input bool     UseAutoBreakEven    = true;       // Activar Auto Break-Even (Activado V1.015)
...
input string   Trend_Settings    = "--- FILTRO DE TENDENCIA MACRO (Alex Ruiz: H1 o superior) ---";
input bool     UseTrendFilter    = true;       // Activar filtro de tendencia macro
input ENUM_TIMEFRAMES TrendTF    = PERIOD_H4;  // Temporalidad de Tendencia (H4 o superior)
input int      TrendEMAPeriod    = 50;         // Periodo EMA de tendencia (50 H4 = ~8 días de lookback)
...
//+------------------------------------------------------------------+
//| Evaluar la tendencia macro basada en EMA200 en H4               |
//| Si precio > EMA200 H4: mercado alcista -> solo COMPRAS           |
//| Si precio < EMA200 H4: mercado bajista -> solo VENTAS            |
//+------------------------------------------------------------------+
bool IsTrendAligned(bool isBuy)
  {
   if(!UseTrendFilter) return true;
   
   double ema200 = iMA(Symbol(), TrendTF, TrendEMAPeriod, 0, MODE_EMA, PRICE_CLOSE, 1);
   double currentPrice = iClose(Symbol(), TrendTF, 1);
   
   if(isBuy)
     {
      // Tendencia alcista: precio por encima de la EMA200 en H4
      return (currentPrice > ema200);
     }
   else
     {
      // Tendencia bajista: precio por debajo de la EMA200 en H4
      return (currentPrice < ema200);
     }
  }

//+------------------------------------------------------------------+
//| Cierre Forzoso de todas las operaciones abiertas al MaxTradeHour |
//+------------------------------------------------------------------+
void CloseAllActiveTrades()
  {
   for(int i = OrdersTotal() - 1; i >= 0; i--)
     {
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
         ...
         if(OrderMagicNumber() == MagicNumber && OrderSymbol() == Symbol())
           {
            bool res = false;
            if(OrderType() == OP_BUY)
               res = OrderClose(OrderTicket(), OrderLots(), Bid, 3, clrOrange);
            else if(OrderType() == OP_SELL)
               res = OrderClose(OrderTicket(), OrderLots(), Ask, 3, clrOrange);
            if(res) Print("Operacion cerrada por MaxTradeHour: ", OrderTicket());
           }
        }
     }
  }
...
void OnTick()
  {
   ...
   // Al llegar a MaxTradeHour: dejar de buscar nuevas entradas y borrar pendientes
   // Las operaciones ya abiertas siguen corriendo hasta su TP/SL/Trailing natural
   ...
   // Si es el final del día (ej. hora 23), limpiamos ordenes pendientes
   if(currentHour >= 23)
     {
      CleanPendingOrders();
     }
